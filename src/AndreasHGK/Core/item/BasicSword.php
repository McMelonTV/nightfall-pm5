...
class BasicSword extends TieredItem implements Durable, Repairable {

    public function __construct(){
        parent::__construct(self::BASICSWORD, "basicsword", 1);
    }
    ...
    public function getTier(int $tier): Item{
        ...
        switch ($tier){
            case 2:
                ...
                ItemUtils::enchant($item, VanillaEnchantments::SHARPNESS(), 1);
                break;
            case 3:
                ItemUtils::maxDamage($item, 600);
                ItemUtils::variant($item, "3");
                ItemUtils::enchant($item, VanillaEnchantments::SHARPNESS(), 2);
                break;
            case 4:
                ItemUtils::maxDamage($item, 800);
                ItemUtils::variant($item, "4");
                ItemUtils::enchant($item, VanillaEnchantments::SHARPNESS(), 3);
                break;
            case 5:
        ...
        $item->setNamedTag($item->getNamedTag()->setInt("customitem", self::BASICSWORD));
        return $interface->getItem();
    }

    public function getBasicItem() : Item {
        $item = ItemFactory::getInstance()->get(ItemIds::IRON_SWORD, 0, 1);
        $item->setCustomName("§r§bBasic Sword");
        EnchantmentUtils::applyGlow($item);
        ItemUtils::description($item, "§r§7A basic sword, crafted using steeldust.");
