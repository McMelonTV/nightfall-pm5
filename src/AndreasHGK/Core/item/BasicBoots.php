...
class BasicBoots extends TieredItem implements Durable, Repairable {

    public function __construct(){
        parent::__construct(self::BASICBOOTS, "basicboots", 1);
    }
    ...
    public function getTier(int $tier): Item{
        ...
        $item->setNamedTag($item->getNamedTag()->setInt("customitem", self::BASICBOOTS));
        return $interface->getItem();
    }

    public function getBasicItem() : Item {
        $item = ItemFactory::getInstance()->get(ItemIds::IRON_BOOTS, 0, 1);
        $item->setCustomName("§r§bBasic Boots");
        EnchantmentUtils::applyGlow($item);
        ItemUtils::description($item, "§r§7Basic boots, crafted using steeldust.");
