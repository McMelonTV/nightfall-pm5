...
class AdvancedChestplate extends TieredItem implements Durable, Repairable {

    public function __construct(){
        parent::__construct(self::ADVANCEDCHESTPLATE, "advancedchestplate", 1);
    }
    ...
    public function getTier(int $tier): Item{
        ...
        $item->setNamedTag($item->getNamedTag()->setInt("customitem", self::ADVANCEDCHESTPLATE));
        return $interface->getItem();
    }

    public function getBasicItem() : Item {
        $item = VanillaItems::DIAMOND_CHESTPLATE();
        $item->setCustomName("§r§cAdvanced Chestplate");
        EnchantmentUtils::applyGlow($item);
        ItemUtils::description($item, "§r§7A stronger and more durable chestplate.");
