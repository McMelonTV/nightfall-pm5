...
class AdvancedLeggings extends TieredItem implements Durable, Repairable {

    public function __construct(){
        parent::__construct(self::ADVANCEDLEGGINGS, "advancedleggings", 1);
    }
    ...
    public function getTier(int $tier): Item{
        ...
        $item->setNamedTag($item->getNamedTag()->setInt("customitem", self::ADVANCEDLEGGINGS));
        return $interface->getItem();
    }

    public function getBasicItem() : Item {
        $item = VanillaItems::DIAMOND_LEGGINGS();
        $item->setCustomName("§r§cAdvanced Leggings");
        EnchantmentUtils::applyGlow($item);
        ItemUtils::description($item, "§r§7Stronger and more durable leggings.");
