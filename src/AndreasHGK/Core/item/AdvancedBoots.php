...
class AdvancedBoots extends TieredItem implements Durable, Repairable {

    public function __construct(){
        parent::__construct(self::ADVANCEDBOOTS, "advancedboots", 1);
    }
    ...
    public function getTier(int $tier): Item{
        ...
        $item->setNamedTag($item->getNamedTag()->setInt("customitem", self::ADVANCEDBOOTS));
        return $interface->getItem();
    }

    public function getBasicItem() : Item {
        $item = VanillaItems::DIAMOND_BOOTS();
        $item->setCustomName("§r§cAdvanced Boots");
        EnchantmentUtils::applyGlow($item);
        ItemUtils::description($item, "§r§7Stronger and more durable boots.");
